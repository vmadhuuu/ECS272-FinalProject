      ...
      genres.forEach(genre => {
        ['NA_Sales', 'EU_Sales', 'JP_Sales'].forEach(region => {
          if (d.Genre === genre) {
            ...
            // Parse the sales as a float and add it to the current total
            genreRegionSalesMap.set(key, genreRegionSalesMap.get(key) + parseFloat(d[region] || 0));
          }
        });
      });
    });
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      const rect = svg.selectAll()
        .data(data, function(d) { return d.Genre+':'+d.Region; })
        .enter()
        .append("rect")
        ...
        .on("mouseover", function(event, d) {
          d3.select(this)
            ...
            tooltip.transition()
              .duration(200)
              .style("opacity", 2);
            tooltip.html("Sales: " + d.Sales + "<br/>Region: " + d.Region + "<br/>Genre: " + d.Genre)
        ...
        function calculateLuminance(color) {
            ...
            const luminance = 0.2126 * Math.pow(rgb.r / 255, 2.2) +
      ...
      .translateExtent([[0, 0], [width, height]]) // Limit the pan to the size of the heatmap
      .on("zoom", (event) => {
